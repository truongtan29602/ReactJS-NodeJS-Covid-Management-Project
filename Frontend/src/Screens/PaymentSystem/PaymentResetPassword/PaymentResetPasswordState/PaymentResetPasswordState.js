...
import "./PaymentResetPasswordState.css";
import { useRef, useEffect, useCallback } from "react";
import { useSpring, animated } from "react-spring";

const PaymentResetPasswordState = (props) => {
  const { resetState, setResetState, error, setError } = props;
  const PaymentResetPasswordStateModalRef = useRef();
  ...
  const closeModal = (e) => {
    if (PaymentResetPasswordStateModalRef.current === e.target) {
      setResetState(false);
    }
  };
    ...
    <>
      {resetState ? (
        <div
          ...
          onClick={closeModal}
          ref={PaymentResetPasswordStateModalRef}
        >
...
export default PaymentResetPasswordState;
